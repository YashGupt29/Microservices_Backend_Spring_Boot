...
public class CredentialServiceImpl implements CredentialService {
  ...
  public List<CredentialDto> findAll() {
    ...
    return this.credentialRepository.findAll().stream()
  ...
  public CredentialDto update(final Integer credentialId, final CredentialDto credentialDto) {
    ...
    return CredentialMappingHelper.map(
        this.credentialRepository.save(CredentialMappingHelper.map(this.findById(credentialId))));
  }
            ...
            .orElseThrow(
                () ->
                    new UserObjectNotFoundException(
                        String.format(
                            "#### Credential with username: %s not found! ####", username))));
  }
}
